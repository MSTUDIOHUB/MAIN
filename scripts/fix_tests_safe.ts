...
const testFiles = [
  ...
  "tests/node/autofix-loop.test.mjs"
];

const replacementInline = `(fsSync.readFileSync(path.join(workspaceRoot, "src/lib/orchestrator.ts"), "utf8") + "\\n" + fsSync.readFileSync(path.join(workspaceRoot, "src/lib/orchestrator/loop/AgentOrchestrator.ts"), "utf8"))`;

const targetInline = `fsSync.readFileSync(path.join(workspaceRoot, "src/lib/orchestrator.ts"), "utf8")`;

const targetMultiline = `fsSync.readFileSync(
    path.join(workspaceRoot, "src/lib/orchestrator.ts"),
    "utf8",
  )`;
  
const targetMultiline2 = `fsSync.readFileSync(
    path.join(workspaceRoot, "src/lib/orchestrator.ts"),
    "utf8"
  )`;
...
for (const file of testFiles) {
  ...
  // Do strict replacements
  content = content.replaceAll(targetInline, replacementInline);
  content = content.replaceAll(targetMultiline, replacementInline);
  content = content.replaceAll(targetMultiline2, replacementInline);
  
  fs.writeFileSync(file, content);
}

console.log("Fixed tests safely.");
