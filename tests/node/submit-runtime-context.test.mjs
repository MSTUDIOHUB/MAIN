...
test("submit runtime context carries immutable turn parameters and initializes mutable state", () => {
  const input = baseInput();
  const context = createSubmitRuntimeContext(input);
...
test("submit runtime context creates isolated mutable sets per context", () => {
  const first = createSubmitRuntimeContext(baseInput({ turnId: "turn-a" }));
  const second = createSubmitRuntimeContext(baseInput({ turnId: "turn-b" }));
