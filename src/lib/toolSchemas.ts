...
export const TOOL_DEFINITIONS: ToolDefinition[] = [
  SUBMIT_PLAN_CANDIDATE_TOOL_DEFINITION,
  {
    ...
    function: {
      name: "spawn_subagent",
      description: "创建一个全新的一次性子智能体来执行一个不可变、语义独立且窄于父目标的任务。协作是可选优化：简单任务可以创建 0 个；仅在并行收益明确时创建。任务结束后实例会永久关闭，后续工作必须重新分析并创建新任务，不得复用旧 agent。相同语义任务会被 runtime 去重；路径只用于权限和租约隔离，不用于生成目标。子智能体运行期间主体应继续处理不重叠工作，依赖结果或最终回答前再 wait_subagents。",
      parameters: {
        type: "object",
        properties: {
          ...
          delegation_reason: { type: "string", description: "为什么该任务值得独立委派，以及主体将并行处理什么" },
          success_criteria: { type: "string", description: "可判定的成功标准；多项使用换行分隔" },
          ...
          required_paths: { type: "string", description: "成功标准要求必须覆盖的精确路径；无必查路径时传空字符串" },
          allowed_paths: { type: "string", description: "权限与租约上限，使用逗号分隔；不得用路径目录名代替语义目标，本地任务最多 6 个" },
          access_mode: {
            type: "string",
            enum: ["read", "write"],
            description: "默认 read；write 仅适用于 implement 且必须继承父级 Execute/Goal/已批准 Plan 权限",
          },
