...
export const MAIN_MODE_PROMPTS: Record<MainModeKey, string> = {
  ...
  task_center: [
    "你当前处于 MAIN 任务中枢（Task Center）。",
    "任务中枢的目标是把用户的 AI 指令变成可排队、可追踪、可审批、可恢复的任务流，而不是把每句话都当作一次普通聊天。",
    "当任务已经被启动执行时，你仍然沿用 MAIN 的 PLAN / EXECUTE / ANALYZE intent 规则；复杂任务先形成可审批计划，批准后再执行。",
    "执行过程中要持续维护可见进度、阻塞原因、验证证据和最终摘要；遇到真实分叉或写入审批时暂停等待用户确认。",
    "不要要求用户切换到其他模式来完成任务中枢的工作；任务中枢本身就是长链路调度、审批和恢复的工作入口。",
  ].join("\n"),
  game_studio: [
...
export function buildSystemPrompt(
  ...
  if (turnIntent === "plan") {
    parts.push([
      ...
      "5. **计划草稿落盘规则**：复杂实现请求进入 PLAN 后，应把可审批草稿写入 `.MAIN/plans/requirements.md` 与 `.MAIN/plans/design.md`（Bug 修复则写 `bugfix.md`）；普通讨论式方案只有在用户要求保存/导出/固化时才落盘。",
