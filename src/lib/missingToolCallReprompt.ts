...
const GENERIC_INTENT_PATTERNS = [
  ...
  /我(?:将|会)?(?:通过|使用|调用)\s*`?(?:read_file|read_document|get_file_outline|grep_search|glob_search|list_directory|analyze_tabular_document|query_tabular_document|write_file|replace_in_file|run_command|execute_command|browser_evaluate)`?\s*(?:来)?(?:创建|执行|生成|写入|修改|删除|添加|运行|实现|获取|查看|扫描|读取|分析|检查|搜索|访问|浏览|查找|探索|验证|测试)/i,
