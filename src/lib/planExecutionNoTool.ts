  ...
  if (input.language === "zh") {
    return [
      ...
      input.missingTasksArtifact
        ...
        : "优先处理下面的任务。需要修改文件就调用 `read_file`/`write_file`/`replace_in_file`；一次性命令验证用 `run_command` 并检查 exitCode/stdout/stderr；页面渲染验证必须使用 Browser/Playwright DOM 或截图证据，不能用 curl/grep/cat 代替；Tauri/人工验证不可自动完成时要暂停说明待用户验证。完成任务后，必须先产生真实证据；如果 tasks.md 已存在，再把对应 checkbox 更新为 `[x]`。",
      "当前缺失证据：",
      remainingText,
      commandHint,
    ].filter(Boolean).join("\n");
