...
export function buildPlanExecutionNoToolRecoveryPrompt(input: {
  ...
  if (input.language === "zh") {
    return [
      ...
      input.missingTasksArtifact
        ...
        : "优先处理下面的任务。需要修改文件就调用 `read_file`/`write_file`/`replace_in_file`；需要验证就调用 `run_command` 并检查 exitCode/stdout/stderr。完成任务后，必须先产生真实证据；如果 tasks.md 已存在，再把对应 checkbox 更新为 `[x]`。",
      "当前缺失证据：",
      remainingText,
      commandHint,
    ].filter(Boolean).join("\n");
  ...
  return [
    ...
    input.missingTasksArtifact
      ...
      : "Start with the tasks below. Use `read_file`/`write_file`/`replace_in_file` for file work; use `run_command` for verification and inspect exitCode/stdout/stderr. After real evidence exists, update the matching tasks.md checkbox to `[x]` if tasks.md exists.",
    "Missing evidence:",
    remainingText,
    commandHint,
  ].filter(Boolean).join("\n");
