...
export function hasOnlyNonBlockingPlanReplyOptions(replyOptions: ReplyOption[]): boolean {
  return (
    Array.isArray(replyOptions) &&
    replyOptions.length > 0 &&
    replyOptions.every((option) =>
...
export function buildReadOnlyPermissionContinuationPrompt(language: "zh" | "en", options?: { allowFileRead?: boolean }): string {
  ...
  if (!allowFileRead) {
    return language === "zh"
      ? "用户已允许本会话内后续步骤。当前能力由恢复契约决定，请只使用本轮实际列出的工具推进下一能力，不要根据旧提示猜测工具可用性，也不要再次询问许可。"
      : "The user has allowed execution steps for this session. The active recovery contract owns the current capability; use only the tools listed in this request and do not infer availability from an older prompt or ask for permission again.";
  }
  return language === "zh"
