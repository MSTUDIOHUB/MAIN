---
name: test-flakiness
description: "分析不稳定测试，定位偶发失败根因并提出稳定化方案。"
...
> 2. 检查 CI pipeline output and save a log to `test-results/`
...
| Test | System | Fail Rate | Likely Cause | 建议 |
|------|--------|-----------|--------------|----------------|
...
## 7. Update Regression Suite + 可选 报告 File
...
- **Ask before writing** — both the regression-suite update and the report
  file require explicit approval. On write: 结论: **COMPLETE** — flakiness report written. On decline: 结论: **BLOCKED** — user declined write.
