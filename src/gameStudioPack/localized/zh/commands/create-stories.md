...
> 检查 the filename in the epic's Governing ADRs list, or run `/architecture-decision`
...
- **结论: COMPLETE** — [N] stories written to `production/epics/[epic-slug]/`. Run `/story-readiness` → `/dev-story` to begin implementation.
- **结论: BLOCKED** — user declined. No story files written.
