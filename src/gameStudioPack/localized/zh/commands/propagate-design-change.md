...
description: "当某个设计变更后，检查并同步它对其他 GDD、Story、架构与资产规范的影响。"
...
> "[path] not found. 检查 the path and try again."
...
**Review mode check** — resolve before running TD-CHANGE-IMPACT:
...
Apply `technical-director` as a specialist review with gate **TD-CHANGE-IMPACT** (`.protocols/game-studio/docs/director-gates.md`).

Pass: the full Design Change Impact 报告 from Phase 6 (change summary, all affected ADRs with their Still Valid / Needs Review / Likely Superseded classifications, and recommended actions).
...
- **CONCERNS** → surface the specific ADRs or recommendations flagged; ask the user with options: `Revise the impact assessment` / `Accept with noted concerns` / `Discuss further`
...
If user approved: 结论: **COMPLETE** — change impact report saved.
If user declined: 结论: **BLOCKED** — user declined write.
