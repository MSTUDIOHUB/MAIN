---
name: localize
description: "规划或审查本地化内容、术语一致性和多语言交付要求。"
...
- `validate` — 检查 translations for completeness, placeholders, and length
...
If no subcommand is provided, output usage and stop. 结论: **FAIL** — missing required subcommand.
...
报告 all findings with file paths and line numbers. This mode is read-only — no files are written.
...
If yes, write only the diff (new entries), not a full replacement. 结论: **COMPLETE** — strings extracted and written.
...
报告 validation results grouped by locale and severity. This mode is read-only — no files are written.
...
`glob_search` `assets/audio/vo/[locale]/` for all `.wav`/`.ogg` files. Cross-reference against the VO manifest. 报告:
...
`grep_search` `src/` for VO audio references. Verify each referenced path exists in `assets/audio/vo/[locale]/`. 报告 broken references.
...
报告 findings. Flag BLOCKING issues (content unreadable without fix) vs ADVISORY (cosmetic improvements).
...
检查 current freeze status in `production/localization/freeze-status.md` (if it exists).
