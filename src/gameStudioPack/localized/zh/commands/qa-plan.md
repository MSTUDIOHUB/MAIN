---
name: qa-plan
description: "为 Epic、功能或 Sprint 生成测试计划，覆盖关键路径、边界和风险。"
...
**Argument:** `the text after the slash command in the current user request` (blank = ask the user)
...
**Engine**: [engine name from .protocols/game-studio/docs/technical-preferences.md, or "Not configured"]
...
| Story | Type | Automated Test 必需 | Manual Verification 必需 |
|-------|------|------------------------|------------------------------|
...
- [ ] Story file updated to `Status: 完成` (via `/story-done`)
````
...
- Ask the user for scope selection when no argument is provided.
