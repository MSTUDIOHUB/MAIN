---
name: art-bible
description: "建立美术圣经，统一视觉方向、参考、材质、色彩、镜头感和制作规范。"
argument-hint: "[--review full|lean|solo]"
user-invocable: true
---

## 阶段 0: Parse Arguments and Context 检查
...
**Retrofit mode detection**: `glob_search` `design/art/art-bible.md`. If the file exists:
- Read it in full
...
Section | Status
--------|--------
1. Visual Identity Statement | [完成 / Empty / Placeholder]
2. Color Palette | ...
...
Read `.protocols/game-studio/docs/technical-preferences.md` if it exists — extract performance budgets and engine for asset standard constraints.
...
Ask these decisions sequentially across turns using one flat `<user_options>` block per turn:
...
## 阶段 2: Visual Identity Foundation (Sections 1–4)
...
**Specialist review routing (MANDATORY)**: Apply `art-director` as a specialist review:
...
Present the art-director's draft to the user. Ask the user:
...
**Specialist review routing**: Apply `art-director` as a specialist review with Visual Identity Statement and mood targets. Ask: "Define the shape language for this game. Connect each shape principle back to the visual identity statement and a specific game pillar. Explain what these shape choices communicate to the player emotionally."
...
**Specialist review routing**: Apply `art-director` as a specialist review with Visual Identity Statement and mood targets. Ask: "Design the color system for this game. Every semantic color assignment must be explained — why does this color mean danger/safety/reward in this world? Identify which color pairs might fail colorblind players and specify what backup cues are needed."

Write the approved section to file immediately.

---

## 阶段 3: Production Guides (Sections 5–8)
...
**Specialist review routing**: Apply `art-director` as a specialist review with sections 1–4. Ask: "Define character design direction for this game. Cover: visual archetype for the player character (if any), distinguishing feature rules per character type (how do players tell enemies/NPCs/allies apart at a glance?), expression/pose style targets (stiff/expressive/realistic/exaggerated), and LOD philosophy (how much detail is preserved at game camera distance?)."

Write the approved section to file.

### Section 6: Environment Design Language

**Specialist review routing**: Apply `art-director` as a specialist review with sections 1–4. Ask: "Define the environment design language for this game. Cover: architectural style and its relationship to the world's culture/history, texture philosophy (painted vs. PBR vs. stylized — why this choice for this game?), prop density rules (sparse/dense — what drives the choice per area type?), and environmental storytelling guidelines (what visual details should tell the story without text?)."

Write the approved section to file.

### Section 7: UI/HUD Visual Direction

**Specialist review routing**: apply as separate review passes:
...
Collect both. If they conflict (e.g., art-director wants elaborate diegetic UI but ux-designer flags it would reduce combat readability), surface the conflict explicitly with both positions. Do NOT silently resolve — ask the user to let the user decide.
...
- **`technical-artist`**: Engine-specific hard constraints — poly count budgets per asset category, texture memory limits, material slot counts, importer constraints, anything from the performance budgets in `.protocols/game-studio/docs/technical-preferences.md`
...
After all sections are complete (or the scoped set from Phase 1 is complete), apply `creative-director` as a specialist review with gate **AD-ART-BIBLE** (`.protocols/game-studio/docs/director-gates.md`).
...
- Does `.protocols/game-studio/docs/technical-preferences.md` contain a configured engine (not `[TO BE CONFIGURED]`)? → setup-engine is done, skip that option
...
Ask the user for next steps. Only include options that are genuinely next based on the state check above:
...
- Never draft a section without first applying the relevant agent(s)
