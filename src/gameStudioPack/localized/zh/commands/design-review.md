---
name: design-review
description: "从设计角度审查方案或产出，检查目标一致性、体验质量和设计风险。"
...
- **`full`**: 完成 review — all phases + specialist agent delegation (Phase 3b)
...
**Prior review check:** 检查 whether `design/gdd/reviews/[doc-name]-review-log.md` exists. If it does, read the most recent entry — note what verdict was given and what blocking items were listed. This session is a re-review; track whether prior items were addressed.

---

## 阶段 2: Completeness 检查
...
## 阶段 3: Consistency and Implementability
...
## 阶段 3b: Adversarial Specialist Review (full mode only)
...
- **`systems-designer`**: For every formula in the GDD, plug in boundary values (minimum and maximum plausible inputs). 报告 whether any outputs go degenerate — negative values, division by zero, infinity, or nonsensical results at the extremes.
...
此技能为只读 — no files are written during Phase 4.
...
```
## Review — [YYYY-MM-DD] — 结论: [APPROVED / NEEDS REVISION / MAJOR REVISION NEEDED]
