...
description: "扫描文档、系统和实现之间的不一致、冲突或引用断裂。"
...
## 阶段 1: Parse Arguments and Load Registry
...
Count total registered entries. 报告:
```
...
报告 the in-scope GDD list before scanning.

---

## 阶段 3: Search-First Conflict Scan
...
## 阶段 4: Deep Investigation (Conflicts Only)
...
1. **Which GDD is correct?** 检查 the `source:` field in the registry — the
...
After writing: 结论: **COMPLETE** — consistency check finished.
If conflicts remain unresolved: 结论: **BLOCKED** — [N] conflicts need manual resolution before architecture begins.
