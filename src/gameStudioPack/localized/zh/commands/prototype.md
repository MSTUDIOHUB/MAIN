...
## 阶段 5: Generate Prototype 报告

Draft the report:

```markdown
## Prototype 报告: [Concept Name]
...
The creative director evaluates the prototype result against the game's creative vision and pillars, then confirms, modifies, or overrides the prototyper's PROCEED / PIVOT / KILL recommendation. Their verdict is final. Update the REPORT.md `建议` section if the creative director's verdict differs from the prototyper's.

---

## 阶段 7: Summary and Next Steps
...
结论: **COMPLETE** — prototype finished. 建议 is PROCEED, PIVOT, or KILL based on findings above.
