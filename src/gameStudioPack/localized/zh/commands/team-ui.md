---
name: team-ui
description: "编排多位专家协同处理 UI、HUD 与交互体验相关特性。"
...
- 检查 that visual design preserves accessibility compliance: verify color contrast ratios, and confirm color is never the only indicator of state (shape, text, or icon must reinforce it)
...
- **ux-designer**: Verify implementation matches wireframes and interaction spec. Test keyboard-only and gamepad-only navigation. 检查 accessibility features function correctly.
- **art-director**: Verify visual consistency with art bible. 检查 at minimum and maximum supported resolutions.
...
结论: **COMPLETE** — UI feature delivered through full pipeline (UX spec → visual → implementation → review → polish).
结论: **BLOCKED** — pipeline halted; surface the blocker and its phase before stopping.
