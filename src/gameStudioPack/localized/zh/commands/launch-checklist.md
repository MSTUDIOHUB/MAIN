...
## 阶段 3: Scan Codebase Health

- Count `TODO`, `FIXME`, `HACK` comments and their locations
- 检查 for any `console.log`, `print()`, or debug output left in production code
- 检查 for placeholder assets (search for `placeholder`, `temp_`, `WIP_`)
- 检查 for hardcoded test/dev values (localhost, test credentials, debug flags)

---

## 阶段 4: Generate the Launch Checklist
