---
name: team-live-ops
description: "编排多位专家协同处理活动、运营和长期维护内容。"
argument-hint: "[season name 或 event description]"
user-invocable: true
...
Ask the user to approve the season plan before delegating to production teams. Issue the COMPLETE verdict only after the user approves and no unresolved ethics violations remain. If an ethics violation is unresolved, end with 结论: **BLOCKED**.
...
If a BLOCKED state is unresolvable, end with 结论: **BLOCKED** instead of COMPLETE.
...
结论: **COMPLETE** — season plan produced and handed off for production.
