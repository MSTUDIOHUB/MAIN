...
4. Show the recommended path:
   ...
   **Architecture phase:**
   - `/create-architecture` — produce the master architecture blueprint and 必需 ADR list
   - `/architecture-decision (×N)` — record key technical decisions, following the 必需 ADR list
...
## 阶段 3b: Set Review Mode

检查 if `production/review-mode.txt` already exists.
...
结论: **COMPLETE** — user oriented and handed off to next step.
