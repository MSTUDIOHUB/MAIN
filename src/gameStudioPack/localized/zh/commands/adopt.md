---
name: adopt
description: "将现有项目接入 MAIN GAME STUDIO，梳理当前资产、文档、规则和工作流入口。"
...
**Audit mode:** `the first token after the slash command in the current user request` (blank = `full`)

- **No argument / `full`**: 完成 audit — all artifact types
...
- `.protocols/game-studio/docs/technical-preferences.md` — engine configured?
- `docs/engine-reference/` — engine reference docs present?
- `glob_search` `docs/adoption-plan-*.md` — note the filename of the most recent prior plan if any exist
...
If the project appears fresh (no artifacts at all), ask the user:
...
| 必需 Section | Heading pattern to look for |
|---|---|
| Overview | `## Overview` |
...
1. **Parenthetical status values** — `grep_search` for any Status cell containing
...
Read `.protocols/game-studio/docs/technical-preferences.md`. 检查 each field for `[TO BE CONFIGURED]`:
...
## 阶段 3: Classify and Prioritise Gaps
...
## 阶段 5: Present Summary and Ask to Write
...
Work through these steps in order. 检查 off each item as you complete it.
...
## 阶段 7: Offer First Action

After writing the plan, don't stop there. Pick the single highest-priority gap
and offer to handle it immediately using `<user_options>`. Choose the first
branch that applies:
