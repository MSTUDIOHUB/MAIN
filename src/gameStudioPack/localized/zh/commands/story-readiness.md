...
description: "判断 Story 是否已达到可实现状态，检查验收标准、依赖、设计引用和开放问题。"
argument-hint: "[story 文件路径、'all' 或 'sprint']"
user-invocable: true
---
...
**此技能为只读.** 它不会编辑 story files. It reports findings
and asks whether the user wants help filling gaps.

**Output:** 结论 per story (READY / NEEDS WORK / BLOCKED) with a specific
gap list for each non-ready story.

---

## 阶段 0: Resolve Review Mode
...
报告 the scope before proceeding: "Validating [N] story files."
...
此技能为只读. It never proposes edits or asks to write files.
...
## 阶段 8: Director Gate — Story Readiness Review
