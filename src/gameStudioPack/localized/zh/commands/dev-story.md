---
name: dev-story
description: "选择一个 Story 进入实现流程，并路由到合适的开发专家继续执行。"
...
检查: does the story's embedded Manifest Version match the current manifest header date?
...
3. If any dependency has Status other than `完成` or `Done`:
   - Ask the user:
     - Prompt: "Story '[current story]' depends on '[dependency title]' which is currently [status], not 完成. How do you want to proceed?"
     - Options:
       - `[A] Proceed anyway — I accept the dependency risk`
       - `[B] Stop — I'll complete the dependency first`
       - `[C] The dependency is done but status wasn't updated — mark it 完成 and continue`
   - If [B]: set story status to **BLOCKED** in session state and stop. Do not apply any programmer agent.
   - If [C]: ask "May I update [dependency path] Status to 完成?" before continuing.
...
## 阶段 3: Route to the Right Programmer
...
- Respect all 必需 and Forbidden patterns from the control manifest
...
- [x] [criterion] — implemented in `[file path + function]`
...
- **Load before implementing** — do not start coding until all context is loaded
  (story, TR-ID, ADR, manifest, engine prefs). 未完成 context produces code
