...
**Output:** Updated story file (Status: 完成) + surfaced next story.
...
## 阶段 3: Verify Acceptance Criteria
...
| Story Type | 必需 Evidence | Gate Level |
|---|---|---|
...
- **INADEQUATE** → flag as **BLOCKING**: "QA lead: critical logic is untested. 结论 cannot be COMPLETE until coverage improves. Specific gaps: [list]."
...
## 阶段 6: Present the Completion 报告
...
**必需 evidence**: [unit test file | integration test or playtest | screenshot + sign-off | walkthrough doc | smoke check pass]
...
## 阶段 7: Update Story Status

Ask before writing: "May I update the story file to mark it 完成 and log
the completion notes?"

If yes, edit the story file:

1. Update the status field: `Status: 完成`
...
**Code Review**: [Pending / 完成 / Skipped]
```
...
If no more Must Have stories remain in this sprint (all are 完成 or Blocked):
...
If no more stories are ready but Must Have stories are still In Progress (not 完成):
