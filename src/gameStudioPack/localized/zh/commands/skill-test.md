---
name: skill-test
description: "验证某个技能、协议或工作流是否按预期工作。"
...
### 检查 4 — Collaborative Protocol Language
...
### 检查 7 — Argument Hint Plausibility
...
For a single skill:
```
=== Skill Static 检查: /[name] ===

检查 1 — Frontmatter Fields:    PASS
检查 2 — Multiple Phases:       PASS (7 phases found)
检查 3 — 结论 Keywords:      PASS (PASS, FAIL, CONCERNS)
检查 4 — Collaborative Protocol: PASS ("May I write" found)
检查 5 — Next-Step Handoff:     WARN (no follow-up section found)
检查 6 — Fork Context Complexity: PASS (8 phases, context: fork set)
检查 7 — Argument Hint:         PASS

结论: WARNINGS (1 warning, 0 failures)
...
```
=== Skill Static 检查: All 52 Skills ===

Skill                  | Result       | Issues
-----------------------|--------------|-------
gate-check             | COMPLIANT    |
design-review          | COMPLIANT    |
story-readiness        | WARNINGS     | 检查 5: no handoff
...

Summary: 48 COMPLIANT, 3 WARNINGS, 1 NON-COMPLIANT
Aggregate 结论: N WARNINGS / N FAILURES
```

---

## 阶段 2B: Spec Mode — Behavioral Verifier
...
- 检查 whether the skill requires "May I write" before file writes
- 检查 whether the skill presents findings before requesting approval
- 检查 whether the skill ends with a recommended next step
- 检查 whether the skill avoids auto-creating files without approval
...
Overall 结论: FAIL (1 case failed, 1 warning)
```
...
## 阶段 2D: Category Mode — Rubric Evaluation
...
1. 检查 whether the skill's written instructions clearly satisfy the criterion
...
```
=== Skill Category 检查: /[name] ([category]) ===
...
For each skill:
- 检查 if a spec file exists (use the `spec:` path from catalog, or glob `CCGS Skill Testing Framework/skills/*/[name].md`)
...
- 检查 if a spec file exists (use the `spec:` path from catalog, or glob `CCGS Skill Testing Framework/agents/*/[name].md`)
