...
import { buildToolResultPresentation, formatToolResultPresentationAsText } from "../lib/toolResultPresentation";
...
export default function CollapsibleToolBlock({ toolName, target, message, status }) {
  ...
  const resultPresentation = buildToolResultPresentation({ toolName, message: String(message || ""), language });
  const cleanMessage = formatToolResultPresentationAsText(resultPresentation);
  return (
